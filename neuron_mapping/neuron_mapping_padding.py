...
def neuronGroup(NeuModel , group_size, PE_ability):
	...
	for i in range(0,layer_num):
		...
		for j in range(0,node_num):
			...
			if neu_num >= group_size:
				neu_per_node[j+n] = group_size
				neu_num -= group_size
				for m in range(0, group_size):
					neu_id = j * group_size + m
		...
		layer2node[i] = list
		n = n + node_num
	return n, layer2node, neuron2node, neu_per_node, cycle_per_neu_node

# 产生关于每个节点一共需要等待的神经元的数目，和每个节点的产生的神经元的传输的目的节点的相关信息
def create_node_task_pre(DNNNeuronModel, layer2node, neuron2node, neu_per_node, group_size):
...
def create_node_task(DNNNeuronModel, layer2node, neuron2node, neu_per_node, group_size):
	# node_neu_need[node] = neu_num; node_neu_dst[node][neu_id] = dst_list;
	node_neu_need = {}
	node_neu_dst = {}
	#print(layer2node)
...
def display_output(DNN, node_num, layer2node, cycle_per_neu_node):
	f = open("./output.txt",'w')
	print("------------------S----------------------------------------------------------------", file = f)
...
node_num, layer2node, neuron2node, neu_per_node, cycle_per_neu_node = neuronGroup(NeuronModel, group_size, PE_ability)
print(node_num)
#生成任务文件
node_neu_need, node_neu_dst = create_node_task(NeuronModel, layer2node, neuron2node, neu_per_node, group_size)
create_task_list( "task_resnet", node_num, node_neu_need, node_neu_dst, neu_per_node, cycle_per_neu_node)

# 输出
display_output(DNN1, node_num, layer2node, cycle_per_neu_node)